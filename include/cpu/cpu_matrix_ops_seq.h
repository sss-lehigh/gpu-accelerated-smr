...
void addScalar_seq(float* data, float scalar, size_t n) {
...
void launchAddScalar_seq(float *d_data, float scalar, size_t rows, size_t cols) {
  size_t n = rows * cols;
  ...
  addScalar_seq(d_data, scalar, n);
  }

void launchSubtractScalar_seq(float *d_data, float scalar, size_t rows, size_t cols) {
  size_t n = rows * cols;
  scalar = (-1.0f)* scalar;
...
void multiplyScalar_seq(float* data, float factor, size_t n) {
//   size_t index = blockIdx.x * blockDim.x + threadIdx.x;
//   size_t stride = blockDim.x * gridDim.x;
...
void launchMultiplyScalar_seq(float *d_data, float factor, size_t rows, size_t cols) {
  size_t n = rows * cols;
  ...
  multiplyScalar_seq(d_data, factor, n);
  
  }

// Vectorized kernel for fused multiply-add (y = alpha * x + beta)
void fusedSclarMultiplyAndAdd_seq(float* data, float alpha, float beta, size_t n) {
//   size_t index = blockIdx.x * blockDim.x + threadIdx.x;
//   size_t stride = blockDim.x * gridDim.x;
...
void launchFusedScalarMultiplyAndAdd_seq(float *d_data, float alpha, float beta, size_t rows, size_t cols) {
  size_t n = rows * cols;
  int blockSize = 256;
  ...
  fusedSclarMultiplyAndAdd_seq(d_data, alpha, beta, n);
  }

// Vectorized kernel for matrix-matrix addition (C = A + B)
void matrixAddVectorized_seq(const float* A, const float* B, float* C, size_t n) {
//   size_t index = blockIdx.x * blockDim.x + threadIdx.x;
//   size_t stride = blockDim.x * gridDim.x;
...
void launchMatrixAdd_seq(const float *d_A, const float *d_B, float *d_C, size_t rows, size_t cols) {
  size_t n = rows * cols;
  int blockSize = 256;
  ...
  matrixAddVectorized_seq(d_A, d_B, d_C, n);
  }

// Vectorized kernel for in-place matrix addition (A += B)
void inPlaceMatrixAddVectorized_seq(float* __restrict__ A, const float* __restrict__ B, size_t n) {
//   size_t index = blockIdx.x * blockDim.x + threadIdx.x;
//   size_t stride = blockDim.x * gridDim.x;
...
void launchInPlaceMatrixAdd_seq(float *d_A, const float *d_B, size_t rows, size_t cols) {
  size_t n = rows * cols;
  int blockSize = 256;
  ...
  inPlaceMatrixAddVectorized_seq(d_A, d_B, n);
  }

// Vectorized kernel for matrix-matrix subtraction (C = A - B)
void matrixSubVectorized_seq(const float* __restrict__ A, 
...
void launchMatrixSub_seq(const float *d_A, const float *d_B, float *d_C, size_t rows, size_t cols) {
  size_t n = rows * cols;
  int blockSize = 256;
  ...
  matrixSubVectorized_seq(d_A, d_B, d_C, n);
  }

// Vectorized kernel for in-place matrix subtraction (A -= B)
void inPlaceMatrixSubVectorized_seq(float* __restrict__ A, 
                                                 const float* __restrict__ B, 
                                                 size_t n) {
//   size_t index = blockIdx.x * blockDim.x + threadIdx.x;
//   size_t stride = blockDim.x * gridDim.x;
...
void launchInPlaceMatrixSub_seq(float *d_A, const float *d_B, size_t rows, size_t cols) {
  size_t n = rows * cols;
  int blockSize = 256;
  ...
  inPlaceMatrixSubVectorized_seq(d_A, d_B, n);
  }

// GEMM using Shared Memory Tiling
void sgemmSharedMemoryTiled_seq(const float* __restrict__ A, 
                                             const float* __restrict__ B, 
                                             float* __restrict__ C, 
                                             int M, int N, int K) {
...
void launchSgemm_seq(const float *d_A, const float *d_B, float *d_C, int M, int N, int K) {
...
void sgemmAddFused_seq(const float* __restrict__ A, 
                                    const float* __restrict__ B, 
...
void launchSgemmAddFused_seq(const float *d_A, const float *d_B, const float *d_D, float *d_C, 
                         int M, int N, int K) {
...
void matrixScaleAddVectorized_seq(const float* __restrict__ A, 
                                               const float* __restrict__ B, 
                                               float* __restrict__ C, 
...
void launchMatrixScaleAdd_seq(const float *d_A, const float *d_B, float *d_C, 
  ...
  // Launch the kernel
  matrixScaleAddVectorized_seq(d_A, d_B, d_C, alpha, beta, n);
  
  }

// Optimized GEMM with In-Place Accumulation (C = alpha * (A * B) + beta * C)
void sgemmInPlaceAccumulate_seq(const float* __restrict__ A, 
                                             const float* __restrict__ B, 
                                             float* __restrict__ C, 
                                             int M, int N, int K,
                                             float alpha, float beta) {
...
void launchSgemmAccumulate_seq(const float *d_A, const float *d_B, float *d_C, 
...
void matrixElementwiseMultVectorized_seq(const float* A, const float* B, float* C, size_t n) {
//   size_t index = blockIdx.x * blockDim.x + threadIdx.x;
//   size_t stride = blockDim.x * gridDim.x;
...
void launchElementwiseMatrixMult_seq(const float *d_A, const float *d_B, float *d_C, size_t rows, size_t cols) {
  size_t n = rows * cols;
  int blockSize = 256;
  ...
  matrixElementwiseMultVectorized_seq(d_A, d_B, d_C, n);
  }

// Vectorized kernel for in-place element wise matrix multiplication (A .= B)
void inPlaceMatrixElementwiseMultVectorized_seq(float* __restrict__ A, const float* __restrict__ B, size_t n) {
//   size_t index = blockIdx.x * blockDim.x + threadIdx.x;
//   size_t stride = blockDim.x * gridDim.x;
...
void launchInPlaceMatrixElementwiseMultVectorized_seq(float *d_A, const float *d_B, size_t rows, size_t cols) {
  size_t n = rows * cols;
  int blockSize = 256;
