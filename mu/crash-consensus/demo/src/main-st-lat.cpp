...
int main(int argc, char* argv[]) {
  ...
  const std::string ids = std::getenv("IDS");
  int nr_procs = ids.empty() ? 0 : std::count(ids.begin(), ids.end(), ',') + 1;
  std::cout << "Number processes = " << nr_procs << std::endl;
...
void benchmark(int id, std::vector<int> remote_ids, int times, int payload_size,
  ...
  std::this_thread::sleep_for(std::chrono::seconds(5 + 15 - id));
  ...
  if (id == 1) {
    ...
    for (int i = 0; i < times; i++) {
      ...
      // std::cout << "id_replicated = " << static_cast<int>(id_replicated)
      //       << ", offset = " << offset
      //       << ", loop time (nsec) = " <<
      //       static_cast<long>(loop_time.tv_nsec)
      //       << ", timestamps_ranges.size() = " << timestamps_ranges.size()
      //       << std::endl;
